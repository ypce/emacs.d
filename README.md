...
| Font | Use | Install |
|---|---|---|
| AeonikMono Nerd Font Mono (Light) | default / fixed-pitch / icons | in `fonts/` (untracked; commercial, do not distribute). Register once: open the `.otf` files in Font Book. To rebuild from Aeonik Mono: Nerd Fonts FontPatcher, `fontforge -script font-patcher --complete --mono AeonikMono-<Weight>.otf`, then bake in the Claude Code / Codex UI glyphs (U+23FA, spinner stars, braille) from JuliaMono: `fontforge -lang=py -script scripts/patch-font-glyphs.py <juliamono-ttf-dir>` |
